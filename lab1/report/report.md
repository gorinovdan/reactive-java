...
Здесь при малых и средних объёмах данных разница несущественна. При 250 000 чеков виден рост времени во всех реализациях, при этом `stream` и `collector` немного уступают итеративному варианту (`circle`). Вероятная причина — дополнительные проходы и создание промежуточных структур в Stream API и кастомном коллекторе. Алгоритмически задача требует накопления и усреднения, что хорошо ложится на императивный (цикл) подход с минимальными накладными расходами.
